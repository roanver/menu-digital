...
// min/max row height in px — used by JS auto-sizer
$minRowPx    = $density === 'compact' ? 36 : 48;
$maxRowPx    = $density === 'compact' ? 72 : 96;
$catRowPx    = $density === 'compact' ? 28 : 36; // category band height
@endphp
<style>
:root{
    --bg:     {{ $bgColor }};
...
/* ─── Overlay: darken + blur sobre imagen de fondo ─── */
.board-overlay{
...
.board-logo{
    height:5.2vh;width:auto;max-width:8vw;
    border-radius:.6vh;object-fit:cover;flex:none;
}
.board-name{
...
.promo-tag{
    ...
    padding:.4vh .9vw;border-radius:.4vh;
    white-space:nowrap;flex:none;
}
.promo-image{
    width:9.5vh;height:9.5vh;
    object-fit:cover;border-radius:.9vh;flex:none;
...
.item-old-price{
    font-size:calc(var(--font-price) * .7);
    opacity:.32;
...
/* agotado */
.item-row.oo-dimmed .item-name,
.item-row.oo-dimmed .item-desc{ opacity:.3 }
...
.page-dots{
    display:flex;align-items:center;gap:.5vw;flex:none;
}
.page-dot{
...
.offline-dot{
    ...
    transition:background .8s;z-index:100;
}
...
.expired-strip{
    ...
    font-size:1vh;color:rgba(255,255,255,.28);
...
// Serializar categorías para el auto-sizer JS
$catData = $categories->map(fn($cat) => [
    'name'  => $cat->name,
    'items' => $cat->menuItems->map(fn($item) => [
        'name'        => $item->name,
        'desc'        => $showDesc ? ($item->description ?? '') : '',
        'price'       => $restaurant->show_price ? ('$' . number_format($item->is_promo && $item->promo_price ? $item->promo_price : $item->price, 0, ',', '.')) : '',
        'old_price'   => ($restaurant->show_price && $item->is_promo && $item->promo_price) ? ('$' . number_format($item->price, 0, ',', '.')) : '',
...
<div class="board-wrap" :style="`transform:translate(${shift.x}px,${shift.y}px)`">
    ...
    {{-- Páginas: generadas por JS, aquí el contenedor --}}
    <div class="board-content" id="board-content">
        {{-- .board-page elements inserted by JS --}}
    </div>

    {{-- Footer --}}
    @if($footerMsg || $showQr || true)
...
const SHOW_PRICE  = {{ $restaurant->show_price ? 'true' : 'false' }};

// ─── Formateadores ────────────────────────────────────────────────────────
function fmtPrice(n){ return '$' + n.toLocaleString('es-CL'); }

// ─── Auto-sizer y paginador ───────────────────────────────────────────────
function buildPages() {
    const content   = document.getElementById('board-content');
    const availH    = content.clientHeight;
    const availW    = content.clientWidth / COLS;

    // Contar filas totales: un header por cat + items
    let totalRows = 0;
    CAT_DATA.forEach(c => { totalRows += 1 + c.items.length; });

    // Calcular alto de fila ideal
    let rowH = Math.floor(availH / totalRows);
    rowH = Math.max(MIN_ROW, Math.min(MAX_ROW, rowH));

    // Actualizar CSS variable globalmente
    document.documentElement.style.setProperty('--row-h', rowH + 'px');

    // Calcular cuántas filas caben por columna por página
    const rowsPerCol = Math.floor(availH / rowH);
    const rowsPerPage = rowsPerCol * COLS;

    // Dividir en páginas: cada página es un array de bloques {cat, items[]}
    const pages = [];
    let currentPage = [];
    let currentRows = 0;
    let colRows = 0; // filas en la columna actual
    ...
    CAT_DATA.forEach(cat => {
        ...
        const needed = 1 + cat.items.length; // header + items

        // Si la categoría completa no cabe en el resto de esta página → nueva página
        if (currentRows + needed > rowsPerPage && currentPage.length) {
            newPage();
        }

        // Partir categorías grandes en trozos
        let i = 0;
        while (i < cat.items.length) {
            const remaining = rowsPerPage - currentRows;
            if (remaining <= 1 && currentPage.length) { newPage(); continue; }

            const headerCost = currentRows === 0 || i === 0 ? 1 : 0;
            const take = Math.min(cat.items.length - i, remaining - (i === 0 ? 1 : 0));
            if (take <= 0) { newPage(); continue; }

            currentPage.push({ name: cat.name, showHeader: i === 0, items: cat.items.slice(i, i + take) });
            currentRows += (i === 0 ? 1 : 0) + take;
            i += take;

            if (currentRows >= rowsPerPage) newPage();
        }
    });
    if (currentPage.length) pages.push(currentPage);
...
function renderPages(pages) {
    ...
    pages.forEach((blocks, pi) => {
        ...
        page.className = 'board-page' + (pi === 0 ? ' is-active' : '');
        page.dataset.page = pi;

        blocks.forEach(block => {
            // Wrapper de categoría
            const wrap = document.createElement('div');
            wrap.style.cssText = 'grid-column: span 1; display:flex; flex-direction:column;';
            ...
            block.items.forEach(item => {
                ...
                let ooClass = '';
                if (!item.available) {
                    ooClass = OO_STYLE === 'strikethrough' ? ' oo-strikethrough' : ' oo-dimmed';
                }
...
function boardApp() {
    return {
        ...
        init() {
            ...
            // Construir páginas tras primer paint
