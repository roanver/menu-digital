...
<div class="max-w-[860px]"
    ...
    <form method="POST" action="{{ route('admin.items.update', $item) }}" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-4 items-start">
            ...
            <div class="space-y-4">
                ...
                <div class="bg-white border border-[#E5E7EB] rounded-[14px] shadow-[0_1px_2px_rgba(16,24,40,.04)] p-4">
                    ...
                    <p class="text-[11.5px] text-[#9CA3AF] mb-3">Ej: "Porción grande" +1500. Guardar sin variantes las elimina.</p>
        ...
        <!-- Bottom bar -->
        <div class="sticky bottom-0 mt-4 -mx-4 px-4 py-3 bg-white border-t border-[#E5E7EB] flex items-center justify-between gap-2">
            <!-- Destructive -->
            <form method="POST" action="{{ route('admin.items.destroy', $item) }}" onsubmit="return confirm('¿Eliminar este item permanentemente?')">
                @csrf @method('DELETE')
                <button type="submit"
                        class="inline-flex items-center gap-2 bg-white hover:bg-[#FEF2F2] text-[#DC2626] border border-[#FECACA] rounded-[10px] px-[14px] py-[9px] text-[13px] font-semibold transition-colors">
